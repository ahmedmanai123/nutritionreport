...
public class NutritionReportProcessor {
    ...
    public ByteArrayOutputStream generateReport(Locale locale) throws JRException {
            ...
            nutritionList.add(cholesteral);
            nutritionList.add(sodium);
            nutritionList.add(potassium);
            nutritionList.add(calium);
            nutritionList.add(iron);
            MacroNutrient carbMacroNutient=new MacroNutrient("Carbohydrates",48);
            MacroNutrient fatMacroNutrient=new MacroNutrient("Fat",32);
            MacroNutrient proteinMacroNutient=new MacroNutrient("Protein",20);
            List<MacroNutrient> macroNutrientList=new ArrayList<>();
            macroNutrientList.add(carbMacroNutient);
            macroNutrientList.add(fatMacroNutrient);
            macroNutrientList.add(proteinMacroNutient);

            JRBeanCollectionDataSource nutritionDataSource = new JRBeanCollectionDataSource(nutritionList);
            JRBeanCollectionDataSource macronNutritionDataSource=new JRBeanCollectionDataSource(macroNutrientList);
            System.out.println(macronNutritionDataSource);
            ...
            parameters.put("macroNutrientDataSet",macronNutritionDataSource);
